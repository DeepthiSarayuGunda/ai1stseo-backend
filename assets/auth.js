...
(function() {
  ...
  function init() {
    ...
    // Event delegation: catch Login button clicks regardless of React render timing.
    // This survives React re-renders that destroy direct onclick bindings.
    document.addEventListener('click', function(e) {
      var btn = e.target.closest('button, a');
      if (!btn) return;
      var t = btn.textContent.trim().toUpperCase();
      // Only handle the navbar Login/Sign Up button, not our own modal buttons
      if ((t === 'LOGIN' || t === 'SIGN UP') && !btn.closest('.auth-overlay') && !btn.closest('.auth-user-menu')) {
        e.preventDefault();
        e.stopPropagation();
        if (!isLoggedIn()) {
          showModal('login');
    ...
    // Poll for nav to set up logged-in user menu (needs the nav element).
    // Wait until the Login button itself is in the DOM before running.
    var attempts = 0;
    var interval = setInterval(function() {
      ...
      if (nav) {
        ...
        btns.forEach(function(b) {
          ...
          if (txt === 'LOGIN' || txt === 'SIGN UP') found = true;
        });
